...
from whistler.tweeter.secrets import TwitterSecrets

class TwitterAlerter(object):
    def __init__(self, secret: TwitterSecrets):
        self.api = twitter.Api(consumer_key=secret.get_consumer_key(),
    ...
    def send_dm(self, receiving_user_handle: str, msg: str):
        ...
        receiving_user = self.api.GetUser(screen_name=receiving_user_handle, include_entities=False)
        resp = self.api.PostDirectMessage(text=msg, user_id= receiving_user.id, return_json=True)
        print("Twitter alert sent: %s" % resp)
